...
#define BFLOAT162(value) (reinterpret_cast<__nv_bfloat162 *>(&(value))[0])
...
// ElementWise Add grid(N/256),
// block(256) a: Nx1, b: Nx1, c: Nx1, c = elementwise_add(a, b)
__global__ void elementwise_add_bf16_kernel(__nv_bfloat16 *a, __nv_bfloat16 *b,
...
// a: Nx1, b: Nx1, c: Nx1, c = elementwise_add(a, b)
__global__ void elementwise_add_bf16x2_kernel(__nv_bfloat16 *a,
                                              __nv_bfloat16 *b,
                                              __nv_bfloat16 *c, int N) {
  int idx = 2 * (blockIdx.x * blockDim.x + threadIdx.x);
  if (idx < N) {
    __nv_bfloat162 reg_a = BFLOAT162(a[idx]);
    __nv_bfloat162 reg_b = BFLOAT162(b[idx]);
    __nv_bfloat162 reg_c;
    reg_c.x = __hadd(reg_a.x, reg_b.x);
    reg_c.y = __hadd(reg_a.y, reg_b.y);
    BFLOAT162(c[idx]) = reg_c;
  }
}
...
__global__ void elementwise_add_bf16x8_kernel(__nv_bfloat16 *a,
                                              __nv_bfloat16 *b,
                                              __nv_bfloat16 *c, int N) {
  int idx = 8 * (blockIdx.x * blockDim.x + threadIdx.x);
  __nv_bfloat162 reg_a_0 = BFLOAT162(a[idx + 0]);
  __nv_bfloat162 reg_a_1 = BFLOAT162(a[idx + 2]);
  __nv_bfloat162 reg_a_2 = BFLOAT162(a[idx + 4]);
  __nv_bfloat162 reg_a_3 = BFLOAT162(a[idx + 6]);
  __nv_bfloat162 reg_b_0 = BFLOAT162(b[idx + 0]);
  __nv_bfloat162 reg_b_1 = BFLOAT162(b[idx + 2]);
  __nv_bfloat162 reg_b_2 = BFLOAT162(b[idx + 4]);
  __nv_bfloat162 reg_b_3 = BFLOAT162(b[idx + 6]);
  ...
  reg_c_3.y = __hadd(reg_a_3.y, reg_b_3.y);
  if ((idx + 6) < N) {
    BFLOAT162(c[idx + 0]) = reg_c_0;
    BFLOAT162(c[idx + 2]) = reg_c_1;
    BFLOAT162(c[idx + 4]) = reg_c_2;
    BFLOAT162(c[idx + 6]) = reg_c_3;
  }
}
...
__global__ void elementwise_add_bf16x8_pack_kernel(__nv_bfloat16 *a,
                                                   __nv_bfloat16 *b,
                                                   __nv_bfloat16 *c, int N) {
  int idx = 8 * (blockIdx.x * blockDim.x + threadIdx.x);
  // temporary register(memory), .local space in ptx, addressable
...
#pragma unroll
  for (int i = 0; i < 8; i += 2) {
    // __hadd2 for half2 x 4
    BFLOAT162(pack_c[i]) = __hadd2(BFLOAT162(pack_a[i]), BFLOAT162(pack_b[i]));
  }
  // reinterpret as float4 and store 128 bits in 1 memory issue.
  if ((idx + 7) < N) {
    LDST128BITS(c[idx]) = LDST128BITS(pack_c[idx]);
  }
}
